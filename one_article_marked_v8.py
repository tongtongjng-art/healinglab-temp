...
def clean_history_title_source(title, source=""):
    """Clean old archive lines that accidentally stored source URLs in titles."""
    ...
    for sep in [" | ", "｜"]:
        if sep in title:
            ...
            title = left.strip()
            if not source or source == "Daily Reading":
                source = right.strip()
...
def build_xhs_export_page(article, title_zh, paragraph_rows, all_keywords, quote_raw, quote_translated, today, cover_image=""):
    ...
    def load_history_entries(max_count=40):
        ...
        for txt_path in sorted(archive_dir.glob("day-*.txt"), reverse=True):
            ...
            en_title, source_line = clean_history_title_source(en_title, source_line)
...
function cleanHistoryDisplayText(text){{
  return String(text || '').replace(/https?:\/\/\S+/g, '').split(' | ')[0].split('｜')[0].replace(/\s+/g, ' ').trim();
}}
...
function renderFinal(){{
  ...
  const historyItems = (data.history || []).map(h => `
    <a class="history-item" href="${{escapeAttr(h.href || '#')}}" data-topic="${{escapeAttr(h.topic || '其他')}}" data-level="${{escapeAttr(h.level || 'B2')}}">
      ...
      <b>${{escapeHtml(cleanHistoryDisplayText(h.title || '历史外刊'))}}</b>
      <span class="history-source">${{escapeHtml(cleanHistoryDisplayText(h.source || 'Daily Reading') || 'Daily Reading')}}</span>
